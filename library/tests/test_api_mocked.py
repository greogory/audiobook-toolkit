    ...
    def test_rescan_library_success(self, app_client):
        ...
        with patch('subprocess.run', return_value=mock_result):
            with patch('backend.api.PROJECT_ROOT') as mock_root:
                ...
                mock_root.__truediv__ = lambda self, x: MagicMock(__truediv__=lambda s, y: mock_scanner)
    ...
    def test_reimport_database_success(self, app_client):
        ...
        with patch('subprocess.run', return_value=mock_result):
            with patch('backend.api.PROJECT_ROOT') as mock_root:
                ...
                mock_root.__truediv__ = lambda self, x: MagicMock(__truediv__=lambda s, y: mock_import)
    ...
    def test_generate_hashes_success(self, app_client):
        ...
        with patch('subprocess.run', return_value=mock_result):
            with patch('backend.api.PROJECT_ROOT') as mock_root:
                ...
                mock_root.__truediv__ = lambda self, x: MagicMock(__truediv__=lambda s, y: mock_script)
...
class TestMoreMissingEndpoints:
    ...
    def test_audiobooks_with_nonexistent_collection(self, app_client):
        ...
        response = app_client.get('/api/audiobooks?collection=nonexistent_collection_xyz')
        assert response.status_code == 200
...
class TestDuplicatesTitleAuthorLogic:
    ...
    def test_duplicates_excludes_audiobook_author(self, app_client):
        ...
        try:
            ...
            cursor.execute("DELETE FROM audiobooks WHERE author = 'Audiobook' AND title = 'Book With Audiobook Author'")
            conn.commit()
            conn.close()
